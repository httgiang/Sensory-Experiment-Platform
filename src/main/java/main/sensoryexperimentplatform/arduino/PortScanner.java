...
//    }
    ...
    void connectPort(ActionEvent event) {
        ...
        if (selectedPort != null) {
            // Get the actual SerialPort object
            SerialPort portToConnect = SerialPort.getCommPort(selectedPort.getPortName());

            // Try to open the port
            if (portToConnect.openPort()) {
                System.out.println("Successfully connected to " + selectedPort.getPortName());

                // Set up port settings if needed
                portToConnect.setBaudRate(9600); // Adjust this depending on your device's requirements
                portToConnect.setNumDataBits(8);
                portToConnect.setNumStopBits(SerialPort.ONE_STOP_BIT);
                portToConnect.setParity(SerialPort.NO_PARITY);
            } else {
                System.out.println("Failed to open port " + selectedPort.getPortName());
            }
        } else {
            System.out.println("No port selected.");
        }
